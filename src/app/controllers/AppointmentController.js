...
import CancellationMai from '../jobs/CancellationMai';
...
class AppointmentController {
  ...
  async store({ body, userId }, res) {
    ...
    if (!isProvider)
      return res
        .status(400)
        .json({ error: 'You can only appoitments with providers' });

    if (userId === provider_id)
      return res
        .status(400)
        .json({ error: 'User cannot Appointment for yourself' });
    ...
    await Notification.create({
      content: `Novo agendamento de ${isProvider.name} para o dia ${formattedDate}`,
      user: provider_id,
    });
  ...
  async delete({ params, userId }, res) {
    ...
    await Queue.add(CancellationMai.key, {
      appointment,
    });
